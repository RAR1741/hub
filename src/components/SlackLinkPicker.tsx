...
export function SlackLinkPicker({
  slackUserId,
  people,
}: {
  slackUserId: string;
  ...
  async function link() {
    ...
    setOutcome(null);
    try {
      const res = await fetch(`/api/admin/people/${personId}/slack`, {
        method: "PUT",
      ...
      if (res.status === 409) {
        setOutcome({ kind: "error", message: "That Slack user is already linked to someone else." });
      } else {

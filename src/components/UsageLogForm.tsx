...
export function UsageLogForm({ batteries }: { batteries: Battery[] }) {
  ...
  const frc = batteries.find((b) => b.id === batteryId)?.kind === "frc_robot";
    ...
    <form onSubmit={submit} className="flex flex-col gap-3">
      <label className="label">Battery
        <select className="input" value={batteryId} onChange={(e) => setBatteryId(e.target.value)} required>
          {BATTERY_KINDS.map((k) => {
            const inKind = batteries.filter((b) => b.kind === k);
            if (inKind.length === 0) return null;
            return (
              <optgroup key={k} label={BATTERY_KIND_LABELS[k]}>
                {inKind.map((b) => <option key={b.id} value={b.id}>{b.number}{b.model ? ` — ${b.model}` : ""}</option>)}
              </optgroup>
            );
          })}
        </select>

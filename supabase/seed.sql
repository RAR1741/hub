-- on conflict (id) do nothing on every person insert so this file is re-runnable:
-- dev-up.sh re-applies it to heal stacks whose volume predates the mentor/admin rows.
